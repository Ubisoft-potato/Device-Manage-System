...
public class UsersController {
  ...
  public UserInfo getUserInfo(Authentication authentication) {
    ...
    Users user =
        usersService.getOne(
            Wrappers.lambdaQuery(Users.class).eq(Users::getUsername, authentication.getName()));
    List<String> authorities =
        authoritiesService
            .list(Wrappers.lambdaQuery(Authorities.class).eq(Authorities::getUserId, user.getId()))
            .stream()
  ...
  public UserInfo getUserInfoById(@NotBlank @PathVariable String id) {
    return userConverter.toUserInfo(
        usersService.getOne(Wrappers.lambdaQuery(new Users()).eq(Users::getId, id)));
  }
  ...
  public boolean userRegister(@Validated @RequestBody UserInfo userInfo) throws ServiceException {
    ...
    if (Objects.nonNull(
        usersService.getOne(
            Wrappers.lambdaQuery(Users.class).eq(Users::getUsername, user.getUsername())))) {
    ...
    return authoritiesService.saveBatch(
  ...
  public Boolean changePassword(@RequestBody @Validated PasswordReset passwordReset)
    ...
    return usersService.update(
        Wrappers.lambdaUpdate(Users.class)
  ...
  public boolean delete(@NotBlank @PathVariable String id) {
    if (authoritiesService.remove(
        Wrappers.lambdaQuery(Authorities.class).eq(Authorities::getUserId, id))) {
  ...
  public List<String> getNameList(@PathVariable String firstName) {
    if (Objects.equals(firstName, "all")) {
      return usersService.list().stream()
  ...
  public IPage<UserInfo> page(
      @NotNull @RequestBody Page<Users> page, @NotBlank @PathVariable String authority) {
    ...
    return new Page<UserInfo>()
        ...
        .setRecords(
            pageByAuthority.getRecords().stream()
                .map(userConverter::toUserInfo)
                .collect(Collectors.toList()));
  }
