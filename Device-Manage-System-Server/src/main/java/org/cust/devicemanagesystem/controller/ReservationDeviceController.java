...
public class ReservationDeviceController {
  ...
  public boolean save(@RequestBody @Validated ReservationDevice reservationDevice) {
    return reservationDeviceService.save(
        reservationDevice.setFailReason("").setState(ReservationCodeEnum.CHECKING.toString()));
  }
  ...
  public boolean updateById(@RequestBody ReservationDevice reservationDevice) {
    if (Objects.equals(reservationDevice.getState(), "CHECK_SUCCESS")) {
      ...
      Users user =
          usersService.getOne(
              Wrappers.lambdaQuery(Users.class).eq(Users::getId, device.getManager()));
      // 计算当前租用时间
      Duration duration =
          Duration.between(reservationDevice.getStartTime(), reservationDevice.getStopTime());
      costSettlementService.save(
          new CostSettlement()
  ...
  public IPage<ReservationDeviceVo> page(
      @RequestBody @NotNull Page<ReservationDeviceVo> page,
  ...
  @PostMapping("/page")
  @ApiOperation("分页查询预约信息")
  public IPage<ReservationDeviceVo> adminPage(
      @RequestBody @NotNull Page<ReservationDeviceVo> page) {
    return reservationDeviceService.queryReservationPage(page, null, null);
  }

  @Autowired
  public ReservationDeviceController(
      IReservationDeviceService reservationDeviceService,
